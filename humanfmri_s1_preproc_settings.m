...
%     PREPROC.qcdir
%     PREPROC.func_reference_file
%     PREPROC.func_reference_file_masked
%     PREPROC.func_reference_file_binarymask
...
for i = 1:numel(PREPROC.func_bold_files)
    ...
    if ~do_select_run || ismember(i, run_num)
        ...
        if i == 1 % Only first run!
            
            % Select reference image
            PREPROC.func_reference_file = fullfile(PREPROC.preproc_func_dir, 'func_reference.nii');
            if exist(PREPROC.func_sbref_files{i}) % if SBRef exists, it is used as a reference image
                fprintf('Use SBRef image as reference.\n');
                PREPROC.setting_reference_image = 'sbref';
                system(['cp' ...
                    ' ' PREPROC.func_sbref_files{i} ...
                    ' ' PREPROC.func_reference_file]); % Use SBRef image
                ...
                PREPROC.setting_reference_image = 'first_vol';
            ...
            PREPROC.func_reference_file_masked = fullfile(PREPROC.preproc_func_dir, 'func_reference_masked.nii');
            PREPROC.func_reference_file_binarymask = fullfile(PREPROC.preproc_func_dir, 'func_reference_masked_mask.nii');
            system(['export FSLOUTPUTTYPE=NIFTI;' ...
                ...
                'bet' ...
                ' ' PREPROC.func_reference_file ...
...
canlab_preproc_show_montage(PREPROC.func_reference_file_masked, fullfile(PREPROC.qcdir, 'func_reference_masked.png'));
drawnow;
