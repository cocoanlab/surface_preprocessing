...
for i = 1:numel(PREPROC.func_bold_files)
    ...
    if ~do_select_run || ismember(i, run_num)
        ...
        % Apply spatial smoothing for extracting good ICs (final output will be unsmoothed!!!)
        fprintf('Applying spatial smoothing for extracting good ICs (final output will be unsmoothed!!!).\n');
        ...
        system(['export FSLOUTPUTTYPE=NIFTI;' ...
            ...
            ' ' PREPROC.mean_dc_func_bold_files{i} ...
            ' ' num2str(brightness_threshold) ...
            ' ' fullfile(temp_sm_dir, ['temp_sm_' b '.nii'])]);
        ...
        if isfield(PREPROC, 'surfrecon_dir')
            system(['export FSLOUTPUTTYPE=NIFTI_GZ;' ...
                ...
                'python ' fullfile(ica_aroma_basedir, 'ICA_AROMA.py') ...
                ' -i ' fullfile(temp_sm_dir, ['temp_sm_' b '.nii']) ...
